...
Если возникает состояние non fast forward между локальной и удаленной версиями одной ветки:  
`git rebase` - перебазировать локальну ветку на последний коммит в ветке удаленном репозитории  
`git push --force` - перезаписать удаленную ветку локальной копией (ветка в удаленном репозитории полностью заменяется, конфликтные коммиты пропадут!)
