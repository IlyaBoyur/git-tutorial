...
`git diff <branch 1> <branch 2> - показать изменения между ветками  
...
### Клонирование
`git clone git@github.com:YandexPraktikum/first-project.git` - склонировть репозиторий с URL first-project.git из аккаунта YandexPraktikum на мой локальный компьютер  
