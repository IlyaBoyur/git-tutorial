...
Ситуация: вы создали PR (MR) и в процессе ревью кто-то обновил `main` (например, влили другую ветку).  
Вариант А: нет конфликтов слияния - ничего делать не нужно.  
Вариант Б: появились конфликты слияния, тогда:  
```bash
git checkout main
git pull # подтянет свежие изменения из origin
git checkout feature/my-new-awesome-code
git merge main
# решить конфликты слияния, добавить файлы с решенными конфликтами в индекс, закоммитить
git push # отправить новую версию ветви функциональности на то же ревью
```
