...
### Алгоритм-шпаргалка для создания PR на Github
1. Склонировать репозиторий
    1. Если вы не участник проекта, сделать `fork` репозитория
    2. Скопировать ссылку Code -> SSH -> скопировать ссылку
    3. Клонировать `git clone <ссылка на репозиторий>`
2. Cоздать новую ветку `git checkout -b my-task-branch-name`
3. Добавить и «закоммитить» изменения
4. Запушить `git push --set-upstream origin HEAD` или `git push -u origin my-task-branch-name`
    1. GitHub (с помощью Git) выведет ссылку на создание PR. Перейти По ней
    2. PR можно также создать через интерфейс GitHub
5. Сообщить о пул-реквесте ревьюеру.
6. Ревьюер просматривает изменения и предлагает правки, если они необходимы.
...
## Конфликт слияния
Конфликт возникает при слиянии веток, история которых "разошлась" (находятся в режиме non-fast-forward).  
В процессе разрешения конфликта файлы с конфликтами получаются статус  **both modified** (англ. «оба изменены»).  


### Алгоритм-шпаргалка для разрешения конфликтов слияния
1. Выбрать инструмент разрешения конфликта (VS code, vimdiff, вручную, др.)
2. Открыть файл, в котором есть конфликт.
3. Выбрать, какие части файла нужно взять из одной ветки, а какие — из другой
4. Добавить результат в индекс: `git add <file_with_conflict>`
5. Когда конфликты разрешены, сделать коммит: `git commit --no-edit` или `git commit -m "<merge_commit_custom_message>"`


### Алгоритм при конфликте, когда `main` "убежал" вперед во время ревью
Ситуация: вы создали PR (MR) и в процессе ревью кто-то обновил `main` (например, кто-то влил другую ветку).  
Вариант А: нет конфликтов слияния - ничего делать не нужно. 

Вариант Б: появились конфликты слияния, тогда:  
1. `git checkout main`
2. Подтянуть свежие изменения из origin `git pull`
3. `git checkout feature/my-new-awesome-code`
4. `git merge main`
5. Решить конфликты слияния, добавить файлы с решенными конфликтами в индекс, закоммитить
6. Отправить новую версию ветви функциональности на то же ревью `git push`
